...
## Code for pulling EHI data outputs into a single .Rdata file 
...
genome_counts_filt %>%
  ...
  ggplot(., aes(x=sample,y=count, fill=phylum, group=phylum)) + #grouping enables keeping the same sorting of taxonomic units
    ...
    scale_fill_manual(values=phylum_colors) +
    facet_nested(. ~ host_order+host_species,  scales="free") + #facet 
...
phylum_summary <- genome_counts_filt %>%
  ...
  group_by(sample,phylum,region, host_order) %>%
...
phylum_summary %>%
    ...
    summarise(total_mean=mean(relabun*100, na.rm=T),
              ...
              squamate_mean=mean(relabun[host_order=="Squamate"]*100, na.rm=T),
              squamate_sd=sd(relabun[host_order=="Squamate"]*100, na.rm=T),
              rodent_mean=mean(relabun[host_order=="Rodentia"]*100, na.rm=T),
              rodent_sd=sd(relabun[host_order=="Rodentia"]*100, na.rm=T)) %>%
...
family_summary <- genome_counts_filt %>%
  ...
  left_join(sample_metadata, by = join_by(sample == sample)) %>% #append sample metadata
  left_join(., genome_metadata, by = join_by(genome == genome)) %>% #append genome metadata
  group_by(sample, family, host_order) %>%
  summarise(relabun=sum(count))
```
...
family_summary %>%
    ...
    summarise(total_mean=mean(relabun*100, na.rm=T),
              total_sd=sd(relabun*100, na.rm=T),
              squamate_mean=mean(relabun[host_order=="Squamata"]*100, na.rm=T),
              squamate_sd=sd(relabun[host_order=="Squamata"]*100, na.rm=T),
              rodent_mean=mean(relabun[host_order=="Rodentia"]*100, na.rm=T),
              rodent_sd=sd(relabun[host_order=="Rodentia"]*100, na.rm=T))  %>%
...
genus_summary <- genome_counts_filt %>%
  ...
  left_join(sample_metadata, by = join_by(sample == sample)) %>% #append sample metadata
  left_join(genome_metadata, by = join_by(genome == genome)) %>% #append genome metadata
  group_by(sample, phylum, genus, host_order) %>%
  summarise(relabun=sum(count)) %>%
  filter(genus != "g__") %>%
